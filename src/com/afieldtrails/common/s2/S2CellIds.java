/*
 * This file is based on com.google.common.geometry.S2CellId, for
 * which the copyright notice below is for.
 * This version has been modified so that it operates on long values
 * rather than an object that contains a long value, along with other
 * methods that optimize the computation from that library.
 */
 ...
 *
 */
public final strictfp class S2CellIds {
   ...
   */
  public static final long SENTINEL = MAX_UNSIGNED;

  /**
   * An invalid cell id guaranteed to be smaller than any valid cell id.
   * Also represents a result of 'none'.
   */
   ...
   */
  public static long fromFacePosLevel(int face, long pos, int level) {
    return parent((((long) face) << POS_BITS) + (pos | 1), level);
  }
   ...
   */
  public static long fromPoint(double px, double py, double pz) {
    int face = xyzToFace(px, py, pz);
    R2Vector uv = validFaceXyzToUv(face, px, py, pz);
   ...
   * necessarily unit length).
   */
  public static long fromPoint(S2Point p) {
    return fromPoint(p.get(0), p.get(1), p.get(2));
  }

  /** Return the leaf cell containing the given S2LatLng. */
  public static long fromLatLng(@Nullable S2LatLng ll) {
    if (ll == null) {
      return NONE;
    }
  ...
  /** Return the leaf cell containing the given lat and lng. */
  public static long fromLatLng(double lat, double lng) {
    // Flattening the computation to avoid allocating 2 S1Angles and 1 S2LatLng and 2 S2Points.
    // There is still one allocation of a R2Vector.
    double phi = lat * (Math.PI / 180);
    double theta = lng * (Math.PI / 180);
    double cosphi = Math.cos(phi);
    return fromPoint(Math.cos(theta) * cosphi, Math.sin(theta) * cosphi, Math.sin(phi));
  }

  public static S2Point toPoint(long id) {
    return S2Point.normalize(toPointRaw(id));
  }
   ...
   */
  public static S2Point toPointRaw(long id) {
    ...
    int face = toFaceIJOrientation(id, i, j, null);
    // System.out.println("i= " + i.intValue() + " j = " + j.intValue());
    int delta = isLeaf(id) ? 1 : (((i.intValue() ^ (((int) id) >>> 2)) & 1) != 0)
      ? 2 : 0;
  ...
  public static S2LatLng toLatLng(long id) {
    return new S2LatLng(toPointRaw(id));
  }
   ...
   */
  public static long pos(long id) {
   ...
   * level() == MAX_LEVEL).
   */
  public static boolean isLeaf(long id) {
    return ((int) id & 1) != 0;
  }
   ...
   */
  public static int childPosition(long id, int level) {
  ...
  public static long rangeMin(long id) {
    return id - (lowestOnBit(id) - 1);
  }

  public static long rangeMax(long id) {
    return id + (lowestOnBit(id) - 1);
  }


  /** Return true if the given cell is contained within this one. */
  public static boolean contains(long id, long other) {
    // assert (isValid() && other.isValid());
    return greaterOrEquals(other, rangeMin(id)) && lessOrEquals(other, rangeMax(id));
  }

  /** Return true if the given cell intersects this one. */
  public static boolean intersects(long id, long other) {
    // assert (isValid() && other.isValid());
    return lessOrEquals(rangeMin(other), rangeMax(id))
      && greaterOrEquals(rangeMax(other), rangeMin(id));
  }
   ...
   * around from the last face to the first or vice versa.
   */
  public static long prev(long id) {
    return id - (lowestOnBit(id) << 1);
  }

  /**
   * Return the cell id at the same level along the Hilbert curve advanced by
   * {@code numberOfCells}.  This is equivalent to calling {@link #next} this many
   * times.
   */
  public static long advance(long id, long numberOfCells) {
    return id + numberOfCells * (lowestOnBit(id) << 1);
  }
   ...
   */
  public static long nextWrap(long id) {
    long n = next(id);
    if (unsignedLongLessThan(n, WRAP_OFFSET)) {
      return n;
    }
   ...
   * child_begin(), child_end(), Begin(), or End().
   */
  public static long prevWrap(long id) {
    long p = prev(id);
    if (p < WRAP_OFFSET) {
      return p;
    }
  ...
  public static long begin(int level) {
    return childBegin(fromFacePosLevel(0, 0, 0), level);
  }

  public static long end(int level) {
    return childEnd(fromFacePosLevel(5, 0, 0), level);
  }
   ...
   */
  public static String toToken(long id) {
    if (id == 0) {
      return "X";
    }
   ...
   */
  // TODO(lacz): android support annotations
  public static void getEdgeNeighbors(long id, long neighbors[]) {
    ...
    neighbors[0] = parent(fromFaceIJSame(face, i.intValue(), j.intValue() - size,
      j.intValue() - size >= 0), level);
    neighbors[1] = parent(fromFaceIJSame(face, i.intValue() + size, j.intValue(),
      i.intValue() + size < MAX_SIZE), level);
    neighbors[2] = parent(fromFaceIJSame(face, i.intValue(), j.intValue() + size,
      j.intValue() + size < MAX_SIZE), level);
    neighbors[3] = parent(fromFaceIJSame(face, i.intValue() - size, j.intValue(),
      i.intValue() - size >= 0), level);
  }
   ...
   */
  public static void getVertexNeighbors(long id, int level, List<Long> output) {
    ...
    output.add(parent(id, level));
    output.add(parent(fromFaceIJSame(face, i.intValue() + ioffset, j.intValue(), isame), level));
    output.add(parent(fromFaceIJSame(face, i.intValue(), j.intValue() + joffset, jsame), level));
    ...
    if (isame || jsame) {
      output.add(parent(fromFaceIJSame(face, i.intValue() + ioffset,
        j.intValue() + joffset, isame && jsame), level));
    }
  }
   ...
   */
  public static void getAllNeighbors(long id, int nbrLevel, List<Long> output) {
    MutableInteger i = new MutableInteger(0);
    MutableInteger j = new MutableInteger(0);
    ...
    for (int k = -nbrSize;; k += nbrSize) {
      ...
      if (k < 0) {
        ...
        output.add(parent(fromFaceIJSame(face, i.intValue() + k,
          j.intValue() - nbrSize, j.intValue() - size >= 0), nbrLevel));
        output.add(parent(fromFaceIJSame(face, i.intValue() + k, j.intValue() + size,
          j.intValue() + size < MAX_SIZE), nbrLevel));
      }
      // East, West, and Diagonal neighbors.
      output.add(parent(fromFaceIJSame(face, i.intValue() - nbrSize,
        j.intValue() + k, sameFace && i.intValue() - size >= 0),
        nbrLevel));
      output.add(parent(fromFaceIJSame(face, i.intValue() + size, j.intValue() + k,
        sameFace && i.intValue() + size < MAX_SIZE), nbrLevel));
      if (k >= size) {
        break;
      }
    }
   ...
   */
  public static int toFaceIJOrientation(long id, MutableInteger pi, MutableInteger pj,
    ...
    if (orientation != null) {
      ...
      if ((lowestOnBit(id) & 0x1111111111111110L) != 0) {
        bits ^= S2.SWAP_MASK;
      }
  ...
  private static int getBits1(long id, MutableInteger i, MutableInteger j, int k, int bits) {
  ...
  public static long lowestOnBit(long id) {
    return id & -id;
  }
   ...
   */
  private static long fromFaceIJWrap(int face, int i, int j) {
   ...
   */
  public static long fromFaceIJSame(int face, int i, int j,
                                         boolean sameFace) {
    if (sameFace) {
      return fromFaceIJ(face, i, j);
    } else {
  ...
  // TODO(lacz): Just use UnsignedLong types?
  /**
  ...
  // TODO(lacz): refactor away
  public static boolean lessThan(long x1, long x2) {
    return unsignedLongLessThan(x1, x2);
  }

  // TODO(lacz): refactor away
  public static boolean greaterThan(long x1, long x2) {
    return unsignedLongGreaterThan(x1, x2);
  }

  // TODO(lacz): rename to unsignedLongLessThanOrEquals
  public static boolean lessOrEquals(long x1, long x2) {
    return unsignedLongLessThan(x1, x2) || x1 == x2;
  }

  // TODO(lacz): rename to unsignedLongGreaterThanOrEquals
  public static boolean greaterOrEquals(long x1, long x2) {
    return unsignedLongGreaterThan(x1, x2) || x1 == x2;
  }

  public static String toString(long id) {
    return "(face=" + face(id) + ", pos=" + Long.toHexString(pos(id)) + ", level="
      + level(id) + ")";
  }
  ...
  public static int compareTo(long x1, long x2) {
    return unsignedLongLessThan(x1, x2) ? -1 :
        unsignedLongGreaterThan(x1, x2) ? 1 : 0;
  }

  /*
   * An version of com.google.common.geometry.S2Projections#validFaceXyzToUv using only
   * primitives.
   */
  public static R2Vector validFaceXyzToUv(int face, double px, double py, double pz) {
  ...
  /*
   * A version of com.google.common.geometry.S2Projections#xyzToFace using only primitives.
   */
  public static int xyzToFace(double x, double y, double z) {
